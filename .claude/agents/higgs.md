...
(Registered for Claude Code from `.kiro/agents/higgs.md`, which is the source of
truth -- keep the two in sync. `Bash` is added to the tool list so the merge rate
and the EPA sign-disagreement scan can be MEASURED on real play-by-play rather
than estimated by reading rows; every other instruction below is unchanged.)
