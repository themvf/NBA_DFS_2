"""Backfill nba_matchups (schedule + scores) for a date range.

Calls fetch_schedule() and fetch_scores() for each date that either has no
matchups yet or is missing scores.  Skips dates that already have full data.

Usage:
    python -m ingest.backfill_schedule --start 2025-10-21 --end 2026-04-11
    python -m ingest.backfill_schedule --start 2025-10-21 --end 2026-04-11 --dry-run
"""
...
from ingest.nba_schedule import fetch_schedule, fetch_scores

logger = logging.getLogger(__name__)

SLEEP_BETWEEN_DATES = 3.0  # seconds between ScoreboardV2 calls (NBA API rate limit)
...
def _dates_already_complete(db: DatabaseManager, start: str, end: str) -> set[str]:
    """Dates that have at least one matchup AND all matchups have scores."""
    rows = db.execute(
        """
...
def backfill(
    ...
    # Dates needing schedule fetch: not in DB yet
    need_schedule = [d for d in all_dates if d not in has_matchups]
    # Dates needing score backfill: in DB but scores incomplete
    need_scores = [
    ...
    print(
        f"Schedule backfill | {start} to {end}\n"
    ...
    if skipped:
        print(f"  {skipped} dates skipped due to API errors — re-run to retry them")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Backfill NBA schedule + scores")
