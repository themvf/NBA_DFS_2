...
import {WORKLOAD_POSITIONS,LEGACY_WORKLOAD_POSITIONS,type WorkloadPositions} from '../src/lib/nfl-dfs/workload-selection';
...
async function main(){
  ...
  const snapshot=candidate.input_snapshot as {dkPlayerId:number;position:string;availability:{role:string};workload:unknown;positionWorkload?:unknown}[];
  const size=bs.format==='classic'?9:6;
  for(const l of lineups){const slots=l.slots as Slot[];assert.equal(slots.length,size);assert.equal(new Set(slots.map(s=>s.dkPlayerId)).size,size);assert.ok(Number(l.total_salary)<=50000&&Number(l.total_salary)>=Number(bs.minSalary));for(const slot of slots){const p=snapshot.find(p=>p.dkPlayerId===slot.dkPlayerId)!;assert.ok(p);if(p.position==='QB')assert.equal(p.availability.role,'Expected starter · QB1');if(slot.source==='workload'){assert.ok(p.position==='WR'?p.workload:p.positionWorkload);assert.ok((cs.workloadPositions as WorkloadPositions??LEGACY_WORKLOAD_POSITIONS)[p.position as keyof WorkloadPositions]);}}}
  ...
  const workloadByPosition=Object.fromEntries(WORKLOAD_POSITIONS.map(pos=>[pos,b.reduce((n,l)=>n+(l.slots as Slot[]).filter(s=>s.source==='workload'&&snapshot.find(p=>p.dkPlayerId===s.dkPlayerId)?.position===pos).length,0)]));
  console.log(JSON.stringify({workloadByPosition,baselineId,candidateId,identicalFrozenInputs:true,identicalControls:true,baselineLineups:a.length,workloadLineups:b.length,changedLineups:b.filter((l,i)=>key(l.slots as Slot[])!==key(a[i].slots as Slot[])).length,workloadRosterSlots:direct,legalAndStarterVerified:true,performance:'Pending actual results'},null,2));
}
