...
function stamp(at: number, dated = false) {
  return new Intl.DateTimeFormat("en-US", { ...(dated ? { month: "short", day: "numeric" } as const : {}), hour: "numeric", minute: "2-digit", timeZone: "America/New_York" }).format(at) + " ET";
}
...
export default function MovementIntelligence({ items, selectedKey, onSelect, loading = false, mode = "recent" }: {
  items: MovementInsight[]; selectedKey: string; onSelect: (item: MovementInsight) => void; loading?: boolean; mode?: "recent" | "developing";
}) {
  const [expanded, setExpanded] = useState(false);
  const listId = useId();
  const shown = expanded ? items : items.slice(0, 4);
  return <section className={s.strip} aria-label="Movement intelligence">
    <div className={s.heading}><h2>{mode === "developing" ? "DEVELOPING THIS WEEK" : "MOVEMENT INTELLIGENCE"}</h2><span>{items.length} SIGNAL{items.length === 1 ? "" : "S"} · {mode === "developing" ? "RECORDED CONTEXT" : "RECENT PREMATCH"}</span>
    ...
    <p className={s.policy}>{mode === "developing" ? "Recorded in the past 7 days; direction and support are as of the timestamp, not a live assessment" : "Recorded in the last 30m"} · ordered by recorded book support, then recency · research, not validated picks</p>
    <div className={s.cards} id={listId}>
      {shown.map((item, index) => <button type="button" key={item.key} className={s.card} aria-pressed={item.key === selectedKey} onClick={() => onSelect(item)}>
        ...
        <span className={s.cardFoot}><span>{item.supportLabel}</span><span>OBS {stamp(item.observedAt, mode === "developing")} →</span></span>
      </button>)}
    </div>
    {!items.length && <p className={s.empty}>{loading ? "Checking observation times…" : mode === "developing" ? "No qualifying observations in the past 7 days for upcoming games in this view." : "No recent qualifying movement signals for upcoming games in this view. Earlier observations remain in the signal tape."}</p>}
  </section>;
}
