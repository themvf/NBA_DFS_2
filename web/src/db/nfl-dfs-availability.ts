  ...
  try {
    ...
    for (const row of observations.rows) {
      ...
      if (player) player.injuries = [...(player.injuries ?? []), {id: String(row.id), source: String(row.source), status: String(row.normalized_status),
        practice: row.practice_status ? String(row.practice_status) : null, team: String(row.team), week,
        ...
        hash: String(row.response_hash), reportType: row.report_type ? String(row.report_type) : undefined, kickoff: row.kickoff ? String(row.kickoff) : undefined, url: row.url ? String(row.url) : undefined}];
    }
