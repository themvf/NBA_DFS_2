...
const ODDS_HISTORY_DDLS = [
  ...
  `CREATE TABLE IF NOT EXISTS cfb_historical_game_lines (
      ...
      home_price INTEGER,
      away_price INTEGER,
      line_designation TEXT NOT NULL CHECK (line_designation IN ('historical_reference','source_reported_open','verified_close')),
      ...
      UNIQUE(game_id, provider, market_type, line_designation, raw_payload_hash)
    )`,
  ...
  `CREATE TABLE IF NOT EXISTS cfb_team_game_features (
      ...
      current_weight DOUBLE PRECISION NOT NULL DEFAULT 0 CHECK(current_weight BETWEEN 0 AND 1),
      prior_weight DOUBLE PRECISION NOT NULL DEFAULT 1 CHECK(prior_weight BETWEEN 0 AND 1),
      features_json JSONB NOT NULL DEFAULT '{}'::jsonb,
      source_completeness DOUBLE PRECISION CHECK(source_completeness BETWEEN 0 AND 1),
      created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
  ...
  `CREATE TABLE IF NOT EXISTS cfb_hypotheses (
      ...
      name TEXT NOT NULL,
      claim TEXT NOT NULL,
      status TEXT NOT NULL CHECK(status IN ('PROPOSED','PREREGISTERED','BACKTESTED','HOLDOUT_PASSED','PROSPECTIVE_SHADOW','VALIDATED_SIGNAL','RETIRED')),
...
const NFL_DFS_DDLS = [
 ...
 END IF;
END $$;`,
  // Field ownership from a DraftKings contest export. Mirrors db/schema.py;
  // the Python CLI and this app both write them, so the DDL lives in both.
  `CREATE TABLE IF NOT EXISTS nfl_dfs_field_contests (contest_id TEXT PRIMARY KEY, contest_name TEXT, format TEXT NOT NULL, season INTEGER, week INTEGER, slate_upload_id UUID REFERENCES nfl_dfs_slate_uploads(upload_id) ON DELETE SET NULL, entry_count INTEGER NOT NULL, winning_score DOUBLE PRECISION, median_score DOUBLE PRECISION, min_score DOUBLE PRECISION, file_name TEXT, file_digest TEXT NOT NULL, imported_at TIMESTAMPTZ NOT NULL DEFAULT NOW(), CHECK(format IN ('classic','showdown')), CHECK(entry_count > 0))`,
  `CREATE TABLE IF NOT EXISTS nfl_dfs_field_ownership (id BIGSERIAL PRIMARY KEY, contest_id TEXT NOT NULL REFERENCES nfl_dfs_field_contests(contest_id) ON DELETE CASCADE, player_name TEXT NOT NULL, normalized_name TEXT NOT NULL, drafted_pct DOUBLE PRECISION NOT NULL, drafted_by_slot JSONB NOT NULL DEFAULT '{}'::jsonb, fpts DOUBLE PRECISION, UNIQUE(contest_id, normalized_name))`,
  `CREATE INDEX IF NOT EXISTS idx_nfl_dfs_field_ownership_name ON nfl_dfs_field_ownership (normalized_name)`,
  // The contest's score curve, for ranking your lineups (slate-results.ts).
  // Checked first: ALTER ... IF NOT EXISTS still takes an exclusive lock even
  // when the column exists, which queues every reader behind a busy job.
  `DO $$ BEGIN IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'nfl_dfs_field_contests' AND column_name = 'score_curve') THEN ALTER TABLE nfl_dfs_field_contests ADD COLUMN score_curve JSONB; END IF; END $$`,
