...
export default function ScenarioLab() {
  ...
  return <div className={styles.lab}>
    ...
    <header className={styles.hero}>
      <div><p className={styles.eyebrow}><FlaskConical size={15} /> NFL · DISTRIBUTION RESEARCH</p><h1>Scenario Lab<span>.</span></h1><p className={styles.intro}>See a lineup’s score range and how often it reaches your target.</p></div>
      <div className={styles.heroAside}><span className={styles.researchBadge}>RESEARCH WORKSPACE</span><p>Start with the demo.<br />Inspect one candidate at a time.</p><div className={styles.heroLines} aria-hidden="true">{[30, 48, 79, 100, 91, 65, 39, 19].map((height, i) => <i key={i} style={{ height: `${height}%` }} />)}</div></div>
    </header>

    <section className={styles.quickStart} aria-label="How to use Scenario Lab"><div><strong>1. Run a demo</strong><p>Choose Classic or Showdown, then compare.</p></div><div><strong>2. Inspect a lineup</strong><p>Look at its score range and chance of reaching your target.</p></div><div><strong>3. Compare alternatives</strong><p>Open advanced comparisons when you want more detail.</p></div><Link href="/dfs/nfl">Open your saved Wednesday or Sunday slate and lineups →</Link></section>
    <div className={styles.workspace}>
      <aside className={styles.controls}>
        <form onSubmit={run}>
          <fieldset disabled={busy}><legend>01 <span>Set up a comparison</span></legend>
            <div className={styles.segment} aria-label="Input source"><button type="button" aria-pressed={mode === "demo"} onClick={() => setMode("demo")}>Demo slate</button><button type="button" aria-pressed={mode === "files"} onClick={() => setMode("files")}>Advanced: my files</button></div>
            ...
            <details className={styles.details}><summary>Advanced simulation settings</summary>{mode === "demo" && <label>Draws per bank<select value={draws} onChange={(e) => setDraws(Number(e.target.value))}><option value={500}>500 · quick exploration</option><option value={1000}>1,000 · standard</option><option value={2000}>2,000 · finer comparison</option><option value={3000}>3,000 · extended</option></select></label>}
            <label>Reproducible seed<input type="number" min={0} max={4294967295} required value={seed} onChange={(e) => setSeed(e.target.valueAsNumber)} /></label></details>
        ...
        <details className={styles.method}><summary>How the comparison works</summary><ol><li>Keep the player outcomes fixed.</li><li>Change how outcomes combine.</li><li>Choose on selection draws.</li><li>Inspect separate evaluation draws.</li></ol><p>Ownership, payouts and Kelly allocation are not part of this experiment.</p></details>
      </aside>

      <div className={styles.results}>
        {!result || !selected ? <section className={styles.empty}><div className={styles.emptyIcon}><BarChart3 size={34} /></div><span className={styles.eyebrow}>SEE THE WHOLE LINEUP</span><h2>Start with one demo lineup.</h2><p>Choose a format and select Compare scenarios. These synthetic examples help you learn; they are not your saved DraftKings lineups.</p><div className={styles.emptySteps}><span>100 candidates</span><ArrowRight size={16} /><span>Shared outcomes</span><ArrowRight size={16} /><span>A different picture</span></div><p className={styles.help}>No scores are shown until an experiment has run.</p></section> : <>
          ...
          <details className={styles.details}><summary>Advanced: compare selection methods and all candidates</summary><section aria-label="Policy winners" className={styles.policyGrid}>{result.report.selected.map((policy) => <button key={policy.policy} className={styles.policyCard} aria-pressed={policy.candidateKey === selected.key} onClick={() => setSelectedKey(policy.candidateKey)}><span>{policyNames[policy.policy]}</span><strong>Candidate {result.report.candidates.findIndex((row) => row.key === policy.candidateKey) + 1}</strong><small>{policy.policy === "joint-target-probability" ? pct(policy.selectionObjective) : `${points(policy.selectionObjective)} pts`} <span>selection objective</span></small></button>)}</section>
