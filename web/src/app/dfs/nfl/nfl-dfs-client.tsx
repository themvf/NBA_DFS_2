...
import {DEFAULT_SITUATIONS,type SituationSettings} from '@/lib/nfl-dfs/projection-audit';
...
import { listSavedNflSlates, loadSavedNflWorkspace, loadSavedNflLineups, readNflOptimizerAudit, applyNflComparison, loadNflSalaryCsv, runNflOptimizer, type NflComparisonSource, type NflWorkspaceSlate } from "./actions";
...
import {DEFAULT_WORKLOAD_POSITIONS,type WorkloadPositions} from "@/lib/nfl-dfs/workload-selection";
...
export default function NflDfsClient() {
  ...
  function generate() {
    ...
    const exactExposure = Object.fromEntries(Object.entries(targetExposure).map(([id, percent]) => {
      const targetCount = Math.max(0, Math.min(settings.nLineups, Math.round(percent / 100 * settings.nLineups)));
      return [id, targetCount / settings.nLineups];
    }));
    const payload: NflOptimizerSettings = { ...settings, format: slate.format, lockedPlayerIds: locked, excludedPlayerIds: excluded, minExposureByPlayer: exactExposure, maxExposureByPlayer: exactExposure };
    setShowVisuals(false);
    startTransition(async () => { try { const response = await runNflOptimizer(slate.uploadId, payload); setLineups(response.result.lineups); setSlate(response.slate); setCompletedSettings({ mode: payload.mode, projectionSource: payload.projectionSource, workloadPositions:payload.workloadPositions, situations:payload.situations }); setRunId(response.runId); setSavedRuns((await loadSavedNflWorkspace(slate.uploadId)).runs); setMessage(`Saved optimizer run ${response.runId.slice(0, 8)} with ${response.result.lineups.length}/${settings.nLineups} lineups. ${response.result.warnings.join(" ")}`); } catch (reason) { setError(reason instanceof Error ? reason.message : "Optimizer failed."); } });
  }
  ...
  return <div className="mx-auto max-w-[1600px] space-y-5 p-4 sm:p-6">
    ...
    {slate ? <>
      ...
      <section className="grid gap-5 xl:grid-cols-[minmax(0,1fr)_360px]"><div className="space-y-5">
        <section className="rounded-xl border border-slate-200 bg-white shadow-sm"><div className="flex flex-wrap items-end gap-3 border-b p-4"><div className="mr-auto"><h2 className="font-bold">2. Player pool & controls</h2><p className="text-xs text-slate-500">Click a player name (or its <b>Why?</b> chip) to see the full projection breakdown. Lock or exclude players. OUT/IR players are removed automatically. Our / Player P10 / Player P90 show the historical baseline; calibrated changes appear above.</p></div><label className="flex min-h-10 min-w-56 items-center gap-2 rounded-lg border px-3"><Search className="h-4 w-4 text-slate-400" /><input className="w-full text-sm outline-none" placeholder="Search" value={query} onChange={(e) => setQuery(e.target.value)} /></label><select aria-label="Filter the player pool by position" title={`FLEX shows ${NFL_FLEX_POSITIONS.join(", ")} together — the positions eligible for the DK Classic FLEX slot.`} className="min-h-10 rounded-lg border bg-white px-3 text-sm" value={position} onChange={(e) => { if (isPoolPositionFilter(e.target.value)) { setPosition(e.target.value); setPlayerPage(1); } }}>{POOL_POSITION_FILTERS.map((p) => <option key={p} value={p}>{POOL_FILTER_LABELS[p]} ({positionCounts[p]})</option>)}</select></div>
