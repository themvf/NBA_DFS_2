...
import { appearancesByName, classifyRemoval, findAppearance, teamOffensivePlays,
  VERDICT_LABEL, VERDICT_MARK, type ParticipantRow, type Proposal } from "@/lib/nfl-dfs/removal";
...
/** Why a number missed: did he leave, or did he simply not produce?
 *
 * A proposal, never a finding — participation rows record touches, not snaps,
 * so this is strong evidence at quarterback and weak evidence everywhere
 * else. The wording says so rather than leaving the reader to infer it. */
function RemovalNote({ proposal }: { proposal: Proposal | null }) {
  if (!proposal) {
    return <p className="text-xs text-slate-500">Play-by-play participation has not been loaded for this
      week, so no read on availability is offered. This is not a finding of &ldquo;played normally&rdquo;.</p>;
  }
  const { evidence: e } = proposal;
  return <div className="rounded-lg border border-slate-200 bg-slate-50 p-3">
    ...
    <p className="mt-2 text-xs tabular-nums text-slate-500">
      ...
      {e.teamPlays ? ` · of ${e.teamPlays} team plays` : ""}
    </p>
    <p className="mt-2 text-[11px] text-slate-500">Proposed by {proposal.version} ({proposal.confidence} confidence).
      Nothing is tagged until you confirm it.</p>
  </div>;
}

export default function WeeklyReview({ reports, availableWeeks, season, viewedAt, participants }: { reports: WeeklyReport[]; availableWeeks: number[]; season: number; viewedAt: number; participants: ParticipantRow[] | null }) {
  ...
  // Built once per week, not per row: both are full scans of a week of plays.
  const appearanceIndex = useMemo(() => participants ? appearancesByName(participants) : null, [participants]);
  const teamPlays = useMemo(() => participants ? teamOffensivePlays(participants) : null, [participants]);
  const proposalFor = useMemo(() => (row: ReportRow): Proposal | null => {
    if (!appearanceIndex || !teamPlays) return null;
    const appearance = findAppearance(appearanceIndex, row.name);
    return classifyRemoval({
      position: row.position, appearance,
      teamPlays: teamPlays.get(appearance?.team ?? row.team) ?? [],
      receptions: row.components.find(c => c.stat === "receptions")?.actual ?? null,
    });
  }, [appearanceIndex, teamPlays]);
    ...
    {!report ? <section className="rounded-xl border p-8"><h2 className="font-bold">No saved weekly reports yet</h2><p className="mt-2">The daily report-card job must run for this season. No sample outcomes are substituted.</p></section> : <>
      ...
      <section className="grid gap-4 xl:grid-cols-[minmax(0,1.5fr)_minmax(0,1fr)]">
        ...
        {selected && <aside className="space-y-4 rounded-xl border bg-white p-5"><div><h2 className="text-xl font-bold">{selected.name}</h2><p className="text-xs text-slate-500">{VARIANT_LABELS[variant]} · {selected.forecast?.history_games ?? 0} prior games</p></div>
          ...
          <h3 className="font-semibold">Availability read</h3><RemovalNote proposal={proposalFor(selected)}/>
