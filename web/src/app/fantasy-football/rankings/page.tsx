...
export default async function FantasyRankingsPage({ searchParams }: { searchParams: Promise<{ scoring?: string; window?: string }> }) {
  ...
  return <div className="space-y-5">
    <div className="flex flex-wrap items-end justify-between gap-4">
      ...
      <div className="flex gap-2">{["STD","HALF","PPR"].map((value) => <Link key={value} href={`/fantasy-football/rankings?scoring=${value}&window=${windowHours}`} className={`rounded-lg px-3 py-2 text-sm font-bold ${value===scoring?"bg-slate-900 text-white":"border hover:bg-muted"}`}>{value}</Link>)}<Link href={`/fantasy-football/rankings/print?scoring=${scoring}`} className="rounded-lg border px-3 py-2 text-sm font-bold hover:bg-muted">Print sheet</Link></div>
    </div>
