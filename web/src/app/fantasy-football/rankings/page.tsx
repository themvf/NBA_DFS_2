...
export default async function FantasyRankingsPage({ searchParams }: { searchParams: Promise<{ scoring?: string }> }) {
  ...
  return <div className="space-y-5">
    ...
    <div className="overflow-x-auto rounded-2xl border bg-card"><table className="w-full min-w-[1050px] text-sm"><thead className="bg-muted text-left text-xs uppercase text-muted-foreground"><tr><th className="p-3">Our</th><th className="p-3">Player</th><th className="p-3">Role & signals</th><th className="p-3">Market rank</th><th className="p-3">ADP</th><th className="p-3">Delta</th><th className="p-3">Our proj</th><th className="p-3">Market proj</th><th className="p-3">Games</th><th className="p-3">Conf.</th></tr></thead><tbody>{rankings.map((player) => {
      const rank=player.ourRank??player.ecr; const delta=player.adp!==null&&rank!==null?player.adp-rank:null;
      return <tr key={player.playerId} className="border-t align-top hover:bg-muted/40"><td className="p-3 text-lg font-black">{rank??"—"}</td><td className="p-3"><p className="font-bold">{player.name}</p><p className="text-xs text-muted-foreground">{player.position} · {player.team??"FA"} · Bye {player.byeWeek??"—"}</p></td><td className="max-w-[330px] p-3"><div className="flex flex-wrap gap-1">{player.indicators.slice(0,3).map((badge) => <span key={badge.code} title={JSON.stringify(badge.evidence)} className={`rounded-full px-2 py-1 text-[10px] font-bold ${badgeStyle[badge.class]??"bg-slate-100"}`}>{badge.label}</span>)}</div></td><td className="p-3">{player.ecr??"—"}<span className="block text-xs text-muted-foreground">{player.position}{player.positionRank??""} · T{player.tier??"—"}</span></td><td className="p-3">{player.adp?.toFixed(1)??"—"}</td><td className={`p-3 font-bold ${delta!==null&&delta>=0?"text-emerald-700":"text-red-700"}`}>{delta===null?"—":`${delta>=0?"+":""}${delta.toFixed(1)}`}</td><td className="p-3 font-semibold">{player.ourProjectedPoints?.toFixed(1)??"—"}</td><td className="p-3">{player.projectedPoints?.toFixed(1)??"—"}</td><td className="p-3">{player.expectedGames?.toFixed(1)??"—"}</td><td className="p-3">{player.confidence!==null?`${Math.round(player.confidence*100)}%`:"—"}</td></tr>})}</tbody></table></div>}
  </div>;
}
