...
const LIVE_STEPS = [
  { title: "Recent production", detail: "PPR points per game use the champion model’s position-specific three-season weights. The exact eligible seasons and normalized weights are stored on each player projection." },
  { title: "Sample-aware regression", detail: "The player’s eligible historical-game sample is blended with the champion model’s position-specific equivalent prior games." },
...
export default function ProjectionMethodExplainer({ projectionModel }: { projectionModel: string }) {
  ...
  return <section aria-labelledby="projection-method-heading" className="overflow-hidden rounded-2xl border bg-card shadow-sm">
    <div className="border-b bg-slate-950 px-5 py-4 text-white">
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        <span className="rounded-full bg-blue-500 px-3 py-1 text-xs font-black">{projectionLabel} IS LIVE</span>
      </div>
      <p className="mt-2 max-w-4xl text-sm text-slate-300">The number under <b className="text-white">Our 2026 PPR Base ({projectionLabel})</b> is a 17-active-game season baseline. It is not yet produced by the V2 team-opportunity model.</p>
    </div>

    <div className="grid gap-4 p-5 xl:grid-cols-2">
      <article className="rounded-2xl border border-blue-300 bg-blue-50 p-4">
        <div className="flex flex-wrap items-center justify-between gap-2"><h3 className="text-lg font-black text-blue-950">{projectionLabel} baseline</h3><span className="rounded-full bg-emerald-100 px-2.5 py-1 text-xs font-black text-emerald-800">LIVE NOW</span></div>
    ...
    <div className="grid gap-3 border-t bg-muted/40 p-5 md:grid-cols-3">
      <div className="rounded-xl border bg-card p-3"><p className="text-xs font-black uppercase tracking-wide text-blue-700">Our PPR base</p><p className="mt-1 text-sm">Our live independent {projectionLabel} projection. Open <b>How {projectionLabel} projects</b> on any player for the actual inputs and arithmetic. RB/WR/TE rookies use a draft-pick value curve fit on 2023-2025 rookie outcomes, adjusted by current depth-chart role; QB/K/DST rookies use a position-and-draft-capital prior (a curve showed no improvement there).</p></div>
