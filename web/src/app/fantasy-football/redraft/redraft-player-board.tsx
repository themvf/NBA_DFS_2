...
const RedraftPlayerRow = memo(function RedraftPlayerRow({ player, skillRank, canDraft, onDraft, odds }: PlayerRowProps) {
  ...
  return <>
    ...
    <div role="cell" className="sticky left-0 z-10 border-r bg-card p-3 shadow-[4px_0_8px_-6px_rgba(15,23,42,0.45)]"><p className="font-bold">{player.name}</p><p className="text-xs text-muted-foreground">{displayPosition(player.position)} · {player.team ?? "FA"} · Bye {player.byeWeek ?? "—"}</p><div className="mt-1"><AnalystNoteMarker note={player.analystNote} /></div></div>
