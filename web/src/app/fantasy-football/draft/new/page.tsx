...
export default async function NewFantasyDraftPage() {
  ...
  return <div className="mx-auto max-w-3xl space-y-6"><div><p className="text-xs font-bold uppercase tracking-widest text-emerald-700">Draft setup</p><h1 className="text-3xl font-black">Create a snake draft</h1><p className="text-muted-foreground">Choose a pinned data snapshot. Rankings will not silently change after the draft starts.</p></div>
    ...
    <form action={createFantasyDraft} className="grid gap-5 rounded-2xl border bg-card p-6 sm:grid-cols-2">
      ...
      <label><span className="text-sm font-semibold">Teams</span><input name="teamCount" type="number" min="8" max="14" defaultValue="10" className="mt-1 w-full rounded-lg border bg-background p-2.5" /></label>
      ...
      <label><span className="text-sm font-semibold">Roster format</span><select name="roster" defaultValue="hood-rivals" className="mt-1 w-full rounded-lg border bg-background p-2.5">{Object.entries(ROSTER_PRESETS).map(([key, preset]) => <option key={key} value={key}>{preset.name}</option>)}</select><p className="mt-1 text-xs text-muted-foreground">{ROSTER_PRESETS["hood-rivals"].description}</p></label>
      <label><span className="text-sm font-semibold">Scoring</span><select name="scoring" defaultValue="HALF" className="mt-1 w-full rounded-lg border bg-background p-2.5">{Object.entries(SCORING_PRESETS).map(([key, preset]) => <option key={key} value={key}>{preset.name}</option>)}</select><p className="mt-1 text-xs text-muted-foreground">{SCORING_PRESETS.HALF.description}</p></label>
      <label className="sm:col-span-2"><span className="text-sm font-semibold">Ranking snapshot</span><select name="rankingSetId" defaultValue={rankingSets[0]?.id} className="mt-1 w-full rounded-lg border bg-background p-2.5">{rankingSets.map((set) => <option key={set.id} value={set.id}>{set.scoring} · {set.name}</option>)}</select></label>
