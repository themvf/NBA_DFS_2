...
import type { RosterConfig } from "@/lib/fantasy-football/league-config";
...
function AvailabilityBadge({ odds }: { odds: AvailabilityOdds | null | undefined }) {
  ...
  return <p className={`text-[10px] font-semibold ${tone}`} title={`FFC ADP ${odds.adjustedAdp.toFixed(1)} ± ${odds.adjustedStdev.toFixed(1)} picks · ${odds.sampleSize ?? "few"} drafts sampled (${odds.confidence} confidence)`}>{pct}% avail. next turn</p>;
}
...
export default function DraftRoomClient({ initialState }: { initialState: FantasyDraftState }) {
  ...
  const availabilityById = useMemo(() => new Map(availableWithAdjustments.map((player) => [player.playerId, computeAvailabilityOdds(
  ...
  const draftPlan = useMemo(() => buildDraftPlan({
    recommendations,
    players: availableWithAdjustments.map((player) => ({ playerId: player.playerId, name: player.name, position: player.position, projectedPoints: player.ourProjectedPoints, projectionLow: player.projectionLow, projectionHigh: player.projectionHigh, tier: player.tier, confidence: player.confidence, availabilityProbability: availabilityById.get(player.playerId)?.probability ?? null })),
    roster: myRoster.flatMap((slot) => slot.playerName && slot.position ? [{ name: slot.playerName, position: slot.position, projectedPoints: slot.ourProjectedPoints }] : []),
    rosterConfig, strategy,
  }), [recommendations, availableWithAdjustments, availabilityById, myRoster, rosterConfig, strategy]);
  ...
  const draftPlayer = (playerId: number, playerName: string) => {
    ...
    startTransition(async () => {
      setError(null);
      const selected = draftPlan?.primary.playerId === playerId ? draftPlan.primary : draftPlan?.fallbacks.find((player) => player.playerId === playerId);
      const decision = { strategy, impactLabel: selected?.impactLabel ?? "added to your roster", nextPick, score: selected?.score };
      ...
      setReceipt({ playerName, position: selected?.position ?? "Player", overallPick: draft.currentPick, impactLabel: decision.impactLabel, strategy, nextPick });
      router.refresh();
    });
  };
  ...
  return <div className="space-y-5">
    <header className="sticky top-14 z-30 rounded-2xl border bg-background/95 p-4 shadow-sm backdrop-blur"><div className="flex flex-wrap items-center justify-between gap-4"><div><p className="text-xs font-bold uppercase tracking-widest text-emerald-700">{draft.status} · {DRAFT_STRATEGY_META[strategy].name} strategy</p><h1 className="text-2xl font-black">{draft.name}</h1><p className="text-xs text-muted-foreground">{String(draft.scoringConfig.preset ?? "Custom")} · {draft.teamCount} teams · {draft.roundCount} rounds</p></div><div className="flex gap-6 text-center"><div><p className="text-xs text-muted-foreground">On clock</p><p className="font-black">{current?.teamName ?? "Complete"}</p></div><div><p className="text-xs text-muted-foreground">Current pick</p><p className="font-black">{Math.min(draft.currentPick, board.length)} / {board.length}</p></div><div><p className="text-xs text-muted-foreground">Until yours</p><p className="font-black">{until}<span className="ml-1 text-xs font-normal text-muted-foreground">(#{nextPick})</span></p></div><button disabled={pending || !board.some((slot) => slot.eventId)} onClick={undo} className="rounded-lg border px-3 py-2 text-sm font-semibold disabled:opacity-40">Undo</button></div></div>{error && <p className="mt-3 rounded-lg bg-red-50 p-2 text-sm font-semibold text-red-700">{error}</p>}{showAdjustments && <p className="mt-3 rounded-lg bg-blue-50 p-2 text-xs text-blue-700">📊 {adjustmentDesc}</p>}</header>
