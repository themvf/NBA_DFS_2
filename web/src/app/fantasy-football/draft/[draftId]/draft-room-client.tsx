...
export default function DraftRoomClient({ initialState }: { initialState: FantasyDraftState }) {
  ...
  const availabilityById = useMemo(() => new Map(available.map((player) => [
    player.playerId,
  ...
  ])), [available, draft.currentPick, nextPick, draft.teamCount]);
  const recommendations = useMemo(() => recommendPlayers(available.map((player) => ({ playerId: player.playerId, position: player.position, ourRank: player.ourRank, ecr: player.ecr, adp: player.adp, projectedPoints: player.ourProjectedPoints, tier: player.tier, confidence: player.confidence, availabilityProbability: availabilityById.get(player.playerId)?.probability ?? null })), myRoster.flatMap((slot) => slot.position ? [slot.position] : []), until).slice(0, 5), [available, myRoster, until, availabilityById]);
  ...
  return <div className="space-y-5">
    ...
    <p className="text-center text-xs text-muted-foreground">Our independent rankings use nflverse and Sleeper data. Current ADP from Fantasy Football Calculator is comparison-only. Availability odds are a directional estimate from FFC&apos;s observed ADP mean, variance, and sample size (rescaled to this draft&apos;s team count) &mdash; not a validated betting-style model.</p>
  </div>;
}
