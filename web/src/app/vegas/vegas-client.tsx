...
function MlbLineMovementPanel({ rows }: { rows: MlbLineMovementRow[] }) {
  ...
  return (
    <div className="rounded-lg border bg-white p-4">
      ...
      <p className="text-xs text-gray-500 mb-2">
        Per-game consensus movement from the 30-min odds captures. <span className="font-medium">Pin gap</span> =
        ...
        move (fast synchronized moves are steam; slow drift is position balancing). Per-book columns
        populate from 2026-07-02; earlier consensus history carries the odds-averaging bug and is hidden
        once enough clean rows exist.
      </p>
      {shown.length === 0 ? (
        <div className="rounded bg-amber-50/60 border border-amber-200 px-3 py-2 text-xs text-amber-700">
          Accruing — needs ≥2 pre-game captures per game. Populates from the 30-min capture cron.
        </div>
      ) : (
        <table className="w-full text-xs border-collapse">
          <thead>
            ...
            {shown.map((r, i) => {
              ...
              return (
                <tr key={`${r.gameDate}-${r.matchup}-${i}`}
                    className={`border-b border-gray-50 ${!r.postFix ? "opacity-50" : ""} ${Math.abs(move) >= 3 && r.postFix ? "bg-amber-50/50" : ""}`}>
                  ...
                  <td className={`py-1 text-right tabular-nums font-medium ${move > 0 ? "text-emerald-600" : move < 0 ? "text-red-500" : "text-gray-400"}`}>{pp(move)}</td>
                  ...
                  <td className="py-1 text-right tabular-nums text-gray-500">
                    {r.openTotal != null ? `${r.openTotal.toFixed(1)}${r.totalMove ? ` ${r.totalMove > 0 ? "+" : ""}${r.totalMove.toFixed(2)}` : ""}` : "—"}
                  </td>
                  <td className={`py-1 text-right tabular-nums ${r.pinGapPp != null && Math.abs(r.pinGapPp) >= 1 ? "font-medium text-indigo-600" : "text-gray-500"}`}>
      ...
      {shown.some((r) => !r.postFix) && (
        <p className="text-[11px] text-gray-400 mt-1">* pre-2026-07-02 capture history — movement includes odds-averaging noise</p>
      )}
    </div>
  );
}
  ...
  return (
    <div className="space-y-8 p-6 max-w-5xl mx-auto">
      ...
      {sport === "mlb" && mlbLineMovement && (
        <MlbLineMovementPanel rows={mlbLineMovement} />
      )}
