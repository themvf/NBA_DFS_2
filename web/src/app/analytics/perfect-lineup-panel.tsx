import { getNbaPerfectLineupAnalytics } from "@/db/queries";

const fmt1 = (v: number | null | undefined) => (v == null ? "—" : v.toFixed(1));
const fmt2 = (v: number | null | undefined) => (v == null ? "—" : v.toFixed(2));

export default async function PerfectLineupPanel() {
  const perfectLineupAnalytics = await getNbaPerfectLineupAnalytics();
  if (!perfectLineupAnalytics) return null;

  return (
    <div className="mx-auto mt-8 max-w-5xl rounded-lg border bg-card p-4 space-y-6">
      <div>
        <h2 className="text-sm font-semibold mb-1">Perfect Lineup Structure</h2>
        <p className="text-xs text-gray-500">
          Exact optimal NBA lineups are recomputed from saved historical slates using actual fantasy points,
          salary, and DK roster rules. Team and defense tables are normalized per slate appearance.
        </p>
        <p className="mt-2 text-xs text-gray-400">
          Slates analyzed: {perfectLineupAnalytics.slateCount}
          {" · "}
          Opponent-context slates: {perfectLineupAnalytics.opponentContextSlateCount}
        </p>
      </div>
      ...
      <div>
        ...
        <table className="w-full text-xs">
          ...
          <tbody>
            {perfectLineupAnalytics.summary.map((row) => (
      ...
      <div>
        ...
        <table className="w-full text-xs">
          ...
          <tbody>
            {perfectLineupAnalytics.shapes.slice(0, 12).map((row) => (
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        <div>
          ...
          <table className="w-full text-xs">
            ...
            <tbody>
              {perfectLineupAnalytics.teamRates.slice(0, 15).map((row) => (
        ...
        <div>
          ...
          <table className="w-full text-xs">
            ...
            <tbody>
              {perfectLineupAnalytics.opponentAllow.slice(0, 20).map((row) => (
