...
### RB Schedule Strength — the one context-adjustment feature that shipped (2026-08-05)

**Origin.** Reviewing three candidate refinements to `ourProj`-equivalent FF projections — 2025 defensive stats, coaching changes, and trades — the first two (opponent-quality schedule adjustment, coordinator continuity) were scoped and one was actually built and validated: **RB schedule strength only.** Coaching-change and trade/opportunity-redistribution adjustments were deliberately deferred (the latter is really V2's team-opportunity-allocation job, not a v1.x bolt-on) and are not built.

**The raw-FPA confound, and why it's not a reason to distrust the whole idea.** A mathematician's objection during scoping: a defense that faces the league's best offense twice a season looks worse against that position than it really is — raw "fantasy points allowed by position" is confounded by the strength of schedule *the defense itself* faced, the mirror image of the opponent-adjustment problem `dfs_projections.py` already solves for NBA (`oppDefRtg`/`oppOffFactor`). Fixed with a one-pass residual regression (regress a team's points-allowed-by-position on the average season-long output of the offenses it actually faced; the re-centered residual is the opponent-adjusted rating) — the same idea DVOA is built on, just a cruder team-season version instead of DVOA's play-level model. Confirmed real on 2025 data: for RB specifically, opponent-adjustment nearly doubled the year-over-year R² (0.06 → 0.12) versus the raw number.

**The real finding: opponent-adjustment doesn't matter if the rating doesn't persist year-over-year.** Fitting a pooled AR(1) (`rating_2025 ~ b0 + b1 * rating_2024`, pooled across the 2023→2024 and 2024→2025 transitions, bootstrap 95% CI on `b1`) on real nflverse weekly stats:

| Position | b1 (adjusted) | R² | 95% CI on b1 | Significant? |
|---|---|---|---|---|
| QB | 0.040 | 0.001 | [-0.22, 0.36] | No — indistinguishable from noise |
| **RB** | **0.335** | **0.12** | **[0.15, 0.55]** | **Yes — the only position that clears the bar** |
| WR | -0.060 | 0.004 | [-0.29, 0.16] | No — slightly negative point estimate |
| TE | 0.247 | 0.032 | [-0.12, 0.61] | No — CI straddles zero |

This is consistent with DVOA's own well-documented track record (defensive DVOA is the least year-over-year stable of the three phases) — better opponent-adjustment math wouldn't fix it, because the decay is real personnel/scheme turnover, not measurement noise. **Decision: RB only, per the pre-registered kill criterion (CI must exclude zero). QB/WR/TE are documented null results, not silently dropped** — re-test only once more nflverse seasons exist (n=64 team-position pairs per position across only 2 transitions is thin by this project's usual ≥150-200 bar).

**What's actually applied to a projection.** `SCHEDULE_ADJUSTED_POSITIONS = {"RB"}` in `ingest/ff_independent.py`. For RB, `(target_season - 1)`'s opponent-adjusted rating is projected forward via the frozen AR(1) coefficients (`SCHEDULE_AR1_COEFFICIENTS["RB"] = (14.681, 0.335)`, from `model/ff_schedule_strength_backtest.py` — the same script computed both the validation numbers above and the deployed coefficients, so they can't silently drift apart), averaged across the team's real upcoming-season opponents, normalized to league average, and clamped to `[0.97, 1.03]` — a range derived from the real, observed 2026 factor distribution (0.983–1.015), not guessed. The multiplier applies uniformly across every RB base-points method (history regression, rookie draft curve, rookie/position prior) — it's a fact about the team's opponents, not about the player's own experience, so excluding rookie RBs would have been an arbitrary gap, not a real limitation. `MODEL_VERSION` bumped to `ff-independent-v1.7`. Every projection's `explanation` carries `schedule_strength_factor`, `schedule_strength_evidence` (per-opponent contribution), and — for every position/case where it wasn't applied — an explicit `schedule_strength_skip_reason` (`not_validated_for_qb`, `no_schedule_context`, `opponents_not_published`, etc.), never a silent neutral `1.0`.

**Files:**
- `db/schema.py` — `nfl_defense_vs_position` (raw + opponent-adjusted STD/PPR, all 4 tracked positions persisted for future re-testing even though only RB is applied; rank 1 = stingiest defense, decided explicitly to avoid a sign-flip bug).
- `ingest/ff_defense_stats.py` — computes and persists the ratings from `stats_player_week_{season}.csv` (already-ingested nflverse data, zero new external source). Games-played denominator comes from the schedule file via the shared `build_schedule_context()`, never from counting stat rows (a team-week with no offensive stat line, e.g. a shutout, is still one game played).
- `model/ff_schedule_strength_backtest.py` — the walk-forward AR(1) validation; produces the checked-in `model/ff_schedule_strength_eval.json` artifact the table above is read from.
- `ingest/ff_independent.py` — `build_schedule_context()` (refactored out of `compute_bye_weeks()` so both bye weeks and schedule-strength share one schedule parse instead of two driftable implementations), `schedule_strength_factor()`, `build_schedule_strength_context()`, integration in `project_player()`.
- Tests: `tests/test_ff_defense_stats.py` (games-from-schedule-not-stats, rank direction, regression fallback below `MIN_TEAMS_FOR_REGRESSION`), `tests/test_ff_independent.py` (RB-only application, never-silent skip reasons, clamp/direction, `build_schedule_context`/`compute_bye_weeks` equivalence).

**Operational note:** `ingest.ff_defense_stats` must run (populating `nfl_defense_vs_position` for `target_season - 1`) before `ingest.ff_independent` for the RB factor to apply — if it hasn't, `build_schedule_strength_context()` returns `{}` and every RB gets `schedule_strength_skip_reason="no_schedule_context"` rather than a fabricated neutral factor. Not yet wired into `.github/workflows/fantasy_football_refresh.yml` — add a step there before the next real refresh.

**Not built — deferred, per the pre-registered scope:** coaching-change and trade/opportunity-redistribution adjustments. Both would need new ledgers this feature deliberately didn't build (`nfl_coaching_history`, an effective-dated transaction ledger) and, for trades specifically, real V2 team-opportunity reconciliation rather than a v1.x bolt-on multiplier.
