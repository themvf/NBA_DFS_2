...
- Every materialized independent projection persists a human-readable derivation contract: method, scoring format, season-level points/PPG/weights, weighted-history PPG, position prior and equivalent prior games, regressed PPG, expected games before/after injury, role/injury multipliers, base/final points, and explicitly unmodeled context. The rankings row exposes this through an expandable `How projected` notation; displayed math must come from the persisted projection payload rather than being recomputed in the browser.
...
- Best Ball interaction performance is a contract: player/rank lookup is indexed, drafted membership is set-based, roster validity is calculated once per affected team, and memoized player rows must not rebuild projection explanations after unrelated picks.
...
- FantasyPros is optional legacy market enrichment. A sample/free response must never replace or suppress the independent board.
