...
- The shared redraft/Best Ball roster-aware challenger architecture, Mike Evans changed-team fixture, dependency order, reconciliation gates, and shadow-promotion policy are defined in [`docs/fantasy-football-roster-aware-v2.md`](docs/fantasy-football-roster-aware-v2.md). The executable source provenance, license, field, cadence, history, as-of cutoff, missingness, and fallback contract is [`docs/fantasy-football-v2-source-contracts.md`](docs/fantasy-football-v2-source-contracts.md), implemented by `ingest/ff_source_contracts.py`. The populated 2020-2025 effective roster and team-week foundation, exact opportunity count semantics, coverage totals, canonical digest, and replay command are in [`docs/fantasy-football-v2-historical-context.md`](docs/fantasy-football-v2-historical-context.md), implemented by `ingest/ff_v2_historical_context.py`. The frozen rolling-origin folds, prediction/outcome isolation, four-scope scoring envelope, persisted run metadata, deterministic replay contract, preregistered metrics, and 72 synthetic historical focal-seat ADP baselines are documented in [`docs/fantasy-football-v2-backtest-harness.md`](docs/fantasy-football-v2-backtest-harness.md) and [`docs/fantasy-football-v2-metric-suite.md`](docs/fantasy-football-v2-metric-suite.md), implemented by `model/ff_v2_backtest.py`, `model/ff_v2_metrics.py`, and `model/ff_v2_historical_bestball_cohort.py`. The focal-seat cohorts are counterfactual market baselines, never observed human drafts. V2 is not active and must not change displayed points or ranking order before its chronological promotion gates pass.
...
- **Board membership is guaranteed by MARKET price, not only by our own depth chart — `ff-independent-v1.16` (2026-08-31).** A five-layer coverage audit (roster→`ff_players`→Yahoo match→board→draft pool→team/bye) found three players the market drafts inside the first 132 picks absent from a 440-deep board: **Jordyn Tyson** (our #738 vs Yahoo ADP 95.3), **Jonah Coleman** (130.0), **Keaton Mitchell** (131.8). Same class as the Tuten hole, one layer deeper: `buildDraftPool` unions market-priced players into the rooms, but it can only union from the board, so a player missing at board-build time is unreachable no matter how generous the pool gate is. `_must_include_ids()` now also force-includes anyone whose **earliest price across every market we hold** falls inside `MARKET_GUARANTEE_PICKS = 300` — Best Ball's 240 picks times the same 1.25 ADP-spread headroom `draft-pool.ts` uses, so the two layers agree by construction and the pool gate can only ever narrow what the board already carries. Neither guarantee touches rank or projections (ADP stays comparison-only); they decide only whether a row is *written*. Board 440 → 498; redraft pool 260 → 317, Best Ball 260 → 358, and **0 market-priced players are now excluded from either pool**. Yahoo ADP is a manual paste, so `refresh()` reads the latest capture rather than fetching it, and folds that capture's response hash into `board_digest` — without that the `create_ranking_set` idempotency guard would keep serving a board built against stale prices.
- **Yahoo name aliases are unordered PAIRS, not one-way rewrites (2026-08-31).** `YAHOO_NAME_ALIASES` mapped `kennygainwell → kennethgainwell` because nflverse listed him as Kenneth. nflverse now calls him Kenny, so the rewrite pointed at a spelling that no longer existed and **silently dropped a back the market drafts at pick 118**. Aliases are now declared as `YAHOO_NAME_ALIAS_PAIRS` and expanded into equivalence sets, so a match is attempted under *every* known spelling and a future upstream rename cannot break it again. Standing rule: a name-normalization alias is a claim about two sources disagreeing, never about which one is right — encode it symmetrically.
- **`ff_injuries` crashed the entire scheduled refresh for 5 days (2026-08-31).** `_insert_event` passed `psycopg2.extras.Json()` a state dict read straight from the DB, carrying `expected_return_min`/`expected_return_max` **date** columns; plain `Json()` has no `default=`, so it raised `TypeError: Object of type datetime is not JSON serializable`. `.github/workflows/fantasy_football_refresh.yml` had then failed **13 consecutive scheduled runs** (last success 2026-08-26 14:03) — the board silently froze while the workflow kept reporting a normal failure nobody was reading. Fixed with a `_json()` helper (`dumps=lambda o: json.dumps(o, default=str)`). Two lessons worth keeping: a serializer that works on freshly-parsed source rows can still fail on *round-tripped DB rows*, because the driver hands back real `date`/`datetime` objects where the source had strings; and an ingest step that is incidental to the board (injury enrichment) must not be able to take the whole board build down — the failure had nothing to do with projections.
...
- **Projection Scatter + weekly grid — `/fantasy-football/projections` (2026-08-30).** 2025 actual vs the 2026 projection, tabbed QB/RB/WR/TE/Flex/K/DEF. Both axes share one domain so the parity diagonal is a true 45 degrees, and the domain is NOT zero-anchored (position/correlation, not magnitude). **Per game is the default and the honest view**: the projection is a 17-active-game baseline, so on season totals anyone who missed time in 2025 shows a huge "gain" that is only health (Purdy: +142 total, −0.9 per game). Players with no 2025 sample get a left-hand strip, never a fake zero. DEF and K carry on-page caveats because the model shrinks both — DEF into a ~6-point band, which also makes its mover panels the 2025 table inverted rather than a forecast.
  **Weekly grid** (bottom of the page, all seven tabs) shares the page's position tabs with the chart and reads a new **`ff_player_week_stats`** table, fed from nflverse `stats_player_week_{season}.csv` by `save_weekly_history()` in `ingest/ff_independent.py` (most recent completed season only — the season aggregates need 3 years, a display grid does not, and 3x ~8.6MB per scheduled refresh would be bandwidth for data nothing reads). `ingest/ff_backfill_week_stats.py` fills an existing DB without rebuilding boards. Three rules worth keeping:
  - **A week with no stat line is an ABSENT row, never a stored zero.** The UI renders 0, but "bye", "did not play" and "played and scored nothing" stay distinguishable in the data. Validated end-to-end: all 74 QBs' weekly sums and row counts match `ff_player_season_features` exactly.
  - **Kickers need `yahoo_kicker_points()` per week too.** nflverse's generic `fantasy_points` is 0 for kickers (542 of 543 2025 kicker-weeks), the same gap v1.10 fixed for season totals; the weekly feed does carry the per-distance buckets, so the same helper is reapplied.
  - **`ff_players.bye_week` is the TARGET season's bye and must not be used to annotate a past season.** It differs for most teams (New England: 2026 bye week 11, 2025 bye week 14). The grid derives the shown season's bye from the weekly data itself — the week in which a team put no stat line on the board at all, which resolves to exactly one week for all 32 teams.
  - **DEF weeks come from the TEAM feed, not the player feed.** A team defense has no row in `stats_player_week`, so `save_dst_weekly_history()` reads `stats_team_week_{season}.csv` and reapplies the identical Yahoo components as the season path, with points-allowed tiered per week off the schedule. Verified: every DST scoring component's weekly sum matches the team-season release exactly for all 32 teams, and all 595 players across QB/RB/WR/TE/K/DST reconcile against `ff_player_season_features` on both points and game count.
  **Heat shading is scaled per position group, not absolutely.** Measured over 2025 weeks 1-17 for drafted players, the median week is 16.8 for a QB but 5.0 for a defense and 9.0 for a kicker — DEF's p95 (16.3) sits below QB's median — so one QB-calibrated ramp collapsed ~95% of DEF and K cells into the lightest step. Breakpoints are anchored on each group's own quantiles; RB/WR/TE deliberately SHARE one scale because Flex puts all three in one table. Negative weeks (34 DEF weeks in 2025 under the points-allowed tiers) get a distinct warm step so a −4 is not indistinguishable from a 0.
  Weeks are grouped **Season 1–14 / Playoff 15–17**, matching DK Best Ball's tournament rounds and `ingest/ff_playoff_sos.py`. Week 18 is stored but gets no week column (no standard format scores it); it is carried in a `W18` total so a row still reconciles against the season total on the chart above.
- **Weekly boom/bust panel + the variance screen behind it (2026-08-30).** `/fantasy-football/projections` ends with a per-position boom/bust table built from `ff_player_week_stats` (2023-2025 now backfilled): ceiling (mean of the three best weeks), best week, spike weeks, spike rate, floor (p25), worst week, and streakiness (SD / own mean). Frozen `SPIKE_THRESHOLDS` are each position's p85 weekly score over 2023-2025 — QB 24, RB 18, WR 16, TE 12, K 13, DST 12.
  **The screen result is the durable part, and it is mostly negative.** Correlating a player's metrics year over year (2023→2024 and 2024→2025, ≥8 games in both), AFTER removing what the prior year's points-per-game already tells you:
  - **ceiling beyond PPG: no reliable signal** — WR +0.17 then −0.10, TE +0.27 then −0.07, RB +0.02 then −0.06, QB −0.31 then +0.17. Sign flips between year pairs, which is what noise looks like. A big ceiling is overwhelmingly just "this player is good": raw ceiling repeats at +0.61-0.75, but so does raw mean at +0.75-0.83.
  - **spike rate beyond PPG: weak but consistently signed** — +0.05 to +0.25, one negative.
  - **streakiness beyond PPG: the strongest of the three** — +0.02 to +0.48 for RB/WR/TE, erratic for QB (n=24-25).
  So the panel is labelled descriptive on screen and must stay that way; do not fold ceiling into a projection without a new, separately pre-registered study. QB/K/DST samples (20-32 players) are too small for any claim.
  **Two UI traps found and fixed on first use**, both worth remembering for any variance view: (1) **CV explodes as the mean approaches zero** — sorting by it returned WRs averaging 4-6 points with one big afternoon, so the streak sort holds out players below 40% of the position's spike threshold and says so on screen; (2) **a raw ceiling sort returns the first round**, which is the opposite of the "late upside stash" question it usually gets asked, so an ADP window filter (any / after 60 / 100 / 150) sits beside it.
- **Mock draft rooms were unpickable for market darlings — fixed (2026-08-30).** Both `/fantasy-football/redraft` and `/fantasy-football/best-ball` built their player pool by taking the first 260 of our board and stopping. The cut is OUR rank; the draft being simulated runs on MARKET order, so any player the market drafts early and our model ranks low simply did not exist in the room. Bhayshul Tuten goes at **ADP 52 — round 6 of a 10-team draft** — and sat at our #285, so searching for him returned "0 available players". **11 such players were missing from redraft, 14 from Best Ball**, and they were not random: they are exactly the ascending young players the board buries for want of an opportunity model (Tuten, Jonathon Brooks, Matthew Golden, Blake Corum, Isaiah Likely). The pool is now the union in `web/src/lib/fantasy-football/draft-pool.ts` — our top N **plus anyone whose ADP lands inside the picks that format actually makes** (10x15=150 redraft, 12x20=240 Best Ball) — mirroring the `_must_include_ids` force-include that `ingest/ff_independent.py` already applies when building the board. Board ORDER is untouched: a market darling still appears where our rank puts him, deep in the list; he is merely reachable. Counts went 260 -> 271 and 260 -> 274. Regression test in `scripts/test-redraft.ts`.
  **The first fix was itself wrong in two ways**, both found by a user searching for a player still missing (MarShawn Lloyd), and both now covered by tests: (1) **gating on ONE market is not gating on the market** — FFC-only missed players Yahoo prices far earlier (Aaron Rodgers 180 FFC vs 117 Yahoo, Cam Ward 169 vs 125) and missed Jayden Higgins entirely, who has no FFC price; the gate now takes the earliest price across every market we hold, so a room labelled "Yahoo" is not filtered by somebody else's board. (2) **a hard cutoff at the last pick is a cliff** — Lloyd sat at FFC ADP 150.4 against a 150-pick threshold and was cut by four tenths of a pick; ADP is a central tendency, so the gate carries `DRAFT_POOL_HEADROOM = 1.25`, roughly the one-standard-deviation ADP spread (~18% of ADP) observed on this board. Counts settled at 260 -> 304 (redraft) and 260 -> 287 (Best Ball).
  **Standing rule:** any surface that simulates or supports a real draft must be filtered by BOTH orderings, never by ours alone, and must consult every market it has rather than one. A cut by our rank silently removes the players our model is most likely to be wrong about — precisely the population a mock draft needs to contain — and a cut at exactly the final pick removes players who go earlier than their mean price half the time.
...
- The Best Ball UI must separate model status visibly: the model version stored on the selected ranking snapshot identifies the displayed live 17-active-game baseline, while `V2 challenger — not active` describes the future calculation flow without implying it generates current points. The 2026 live champion is `ff-independent-v1.14`, frozen with STD/HALF/PPR ranking/source identifiers and projection/order digests in `artifacts/ff_champion_baseline_v1.14.json`. Runtime UI, draft-session, and advisor labels must derive from ranking-snapshot metadata rather than a hard-coded version. The champion uses completed-season PPG with position-specific history weights, sample-aware regression, and a bounded depth-chart role factor across 17 active games. True rookies use the documented position-and-draft-capital paths with wider uncertainty. FantasyPros remains comparison-only and ADP remains draft-timing context.
- Execute V2 in this dependency order: immutable 2020–2025 as-of history and reproducible champion/ADP baselines → chronological validation harness → team opportunity → player allocation → efficiency/TD share → weekly distributions/correlations → final validation gate → decision/backend/UI delivery and visual verification.
...
- Both advisors receive the same server-reconstructed, request-hashed evidence contract: DraftKings rules/scoring, the authoritative ranking-snapshot model version, all ordered picks and roster ownership, the user's slot/next pick/roster/bye profile, and the top legal remaining candidates with champion projections, FantasyPros, ADP, prior-season, confidence, and signal fields. Client-supplied projections or model labels are never trusted, providers never see each other's output, and stale responses are discarded after any draft change. Provider-visible payloads omit internal player IDs so the intended selection identifiers are opaque candidate keys (for example, `C01`), which the server maps back to authoritative player IDs. Because DeepSeek JSON mode does not reliably preserve field names, the validator recursively collects only exact live-candidate keys/names, prioritizes winner/primary selection semantics, deduplicates alternatives, and separately finds explanation/strategy/risk fields through bounded nesting. It also normalizes `C1`, snake_case, nested wrappers, `0–1` versus `0–100` confidence, and scalar risk text. When loose output omits structured evidence or alternatives, the UI substitutes server-derived champion projection, ADP, and rank facts and the server's next two legal board candidates, while explicitly labeling omitted confidence, roster fit, risk, or strategy. The primary reference must still map to the current legal candidate set; otherwise one bounded correction retry runs against the same frozen board and then fails closed.
