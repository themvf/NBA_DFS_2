...
- The Best Ball UI must separate model status visibly: `V1.4 — live` identifies the displayed 17-active-game baseline, while `V2 challenger — not active` describes the future calculation flow without implying it generates current points. The live flow is weighted history → sample-aware regression → 17 active games → role adjustment; the challenger flow is team ceiling → role allocation → efficiency/TD share → weekly range → counted-roster impact. FantasyPros remains comparison-only and ADP remains draft-timing context.
- Execute V2 in this dependency order: immutable 2020–2025 as-of history and reproducible V1.4/ADP baselines → chronological validation harness → team opportunity → player allocation → efficiency/TD share → weekly distributions/correlations → final validation gate → decision/backend/UI delivery and visual verification.
...
- V2 promotion evidence must include season-total MAE/bias, weekly CRPS or another preregistered proper scoring rule, P10/P50/P90 coverage, Top-12 precision/recall, spike-week recall, simulated Best Ball counted points, draft-decision regret, and paired bootstrap confidence intervals. Overall season-total bias must be within ±5 points. The final result is `PROMOTE`, `REVISE`, or `RETAIN V1.4`; any FAIL/BLOCKED gate keeps V2 in shadow mode.
- The Best Ball draft room exposes two user-triggered, independent LLM advisors: OpenAI `gpt-5.6-luna` through the Responses API with Structured Outputs and DeepSeek `deepseek-v4-flash` through Chat Completions JSON mode. Credentials are server-only (`OPENAI_API_KEY`, `DEEPSEEK_API_KEY`); provider output never changes the draft unless the user explicitly presses Add.
- Both advisors receive the same server-reconstructed, request-hashed evidence contract: DraftKings rules/scoring, all ordered picks and roster ownership, the user's slot/next pick/roster/bye profile, and the top legal remaining candidates with V1.4, FantasyPros, ADP, prior-season, confidence, and signal fields. Client-supplied projections are never trusted, providers never see each other's output, and stale responses are discarded after any draft change. Provider-visible payloads omit internal player IDs so the intended selection identifiers are opaque candidate keys (for example, `C01`), which the server maps back to authoritative player IDs. The validator safely resolves harmless provider variants (`C1`, text containing a key, an exact legal candidate name, snake_case fields, a nested `recommendation` object, or DeepSeek's loose `pick`/`alternatePicks`/`rationale` shape), normalizes bounded confidence expressed as either `0–1` or `0–100`, and converts a single risk string into its one-item list. When loose DeepSeek output omits structured evidence, the UI substitutes server-derived V1.4 projection, ADP, and rank facts and explicitly labels omitted confidence, roster fit, risk, or strategy. References that cannot map to the current legal candidate set or other malformed output receive one bounded correction retry against the same frozen board, then fail closed with an understandable message.
