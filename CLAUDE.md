...
- **DST (team defense) real projection model, `ff-independent-v1.7` (2026-08-07).** DST previously used a flat, undifferentiated placeholder (`method="position_baseline"`, 105.0 for every team, confidence 0.35) — found while scoping a redraft board clone (below): FantasyPros' own DST projections vary meaningfully by team (real range ~86–120 for 2026), while ours didn't move at all. Replaced with the same 3-year-weighted-history regression every skill position already uses, fed by real per-team data: `ingest/ff_independent.py::save_dst_history()` pulls `stats_team_reg_{season}.csv` (nflverse, confirmed real via a live request, not assumed — season-total sacks/INTs/opponent-fumble-recoveries/safeties/defensive-and-return-TDs per team) and computes points-allowed by mapping each game's actual opponent score (from the same `games.csv` schedule already fetched for bye weeks) through Yahoo's tier table, summed per game across the season — points-allowed is inherently a per-game category, not a season-total lookup. Scoring values verified live against Yahoo's own express-settings default page (not assumed, not a secondary source): Sack +1, INT +2, Fumble Recovery +2, Safety +2, Defensive/Return TD +6, Points Allowed tiered 0=+10 / 1–6=+7 / 7–13=+4 / 14–20=+1 / 21–27=0 / 28–34=−1 / 35+=−4. Deliberately **not modeled: blocked kicks** (+2) — nflverse's team-stats release only records kicks a team's own unit had blocked against it, not blocks credited to its defense against an opponent, which needs game-level opponent cross-referencing this season-aggregate dataset doesn't provide; same "too rare to model, omit" judgment already applied to DK soccer's CG/CGSO/NH scoring elsewhere in this file, documented in `source_row.blocked_kicks_modeled=false` rather than hidden. Result computed against real 2025 data before shipping: team totals ranged 44–158 season points (realistic), correctly separated strong pass-rush/takeaway defenses (SEA 158, HOU 140, DEN 139) from weak ones (NYJ 44, DAL 51, ARI 52), mean 98.2 close to the prior model's flat 105.0 anchor (sanity check that shrinkage behaves sensibly, not evidence of anything more). `project_player()`'s DST special-case was removed entirely — DST now falls through the same `eligible_history` branch as every other position, with the old flat-105.0 `_rookie_points` output remaining only as the true no-history fallback (verified via test, not just left implicit).
