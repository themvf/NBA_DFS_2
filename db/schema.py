...
TABLES = [
    ...
    # and historical references, but does not provide observation timestamps
    # equivalent to the live capture pipeline.
    """
    CREATE TABLE IF NOT EXISTS cfb_historical_game_lines (
        id BIGSERIAL PRIMARY KEY,
        ...
        home_price INTEGER,
        away_price INTEGER,
        line_designation TEXT NOT NULL CHECK (
            line_designation IN ('historical_reference', 'source_reported_open', 'verified_close')
        ),
        ...
        is_canonical_reference BOOLEAN NOT NULL DEFAULT FALSE,
        UNIQUE (
    ...
    """
    CREATE TABLE IF NOT EXISTS cfb_staff_regimes (
        id BIGSERIAL PRIMARY KEY,
        team_id INTEGER NOT NULL REFERENCES cfb_teams(team_id) ON DELETE CASCADE,
        role TEXT NOT NULL CHECK (role IN ('HEAD_COACH', 'OFFENSIVE_COORDINATOR', 'DEFENSIVE_COORDINATOR')),
        ...
        UNIQUE (team_id, role, person_name, start_season, start_week)
    )
    """,

    """
    CREATE TABLE IF NOT EXISTS cfb_roster_snapshots (
        id BIGSERIAL PRIMARY KEY,
        team_id INTEGER NOT NULL REFERENCES cfb_teams(team_id) ON DELETE CASCADE,
        season INTEGER NOT NULL,
        source TEXT NOT NULL,
        ...
        UNIQUE (team_id, season, source, payload_hash)
    )
    """,

    """
    CREATE TABLE IF NOT EXISTS cfb_roster_players (
        id BIGSERIAL PRIMARY KEY,
        snapshot_id BIGINT NOT NULL REFERENCES cfb_roster_snapshots(id) ON DELETE CASCADE,
        ...
        position_group TEXT,
        class_year INTEGER,
        previous_team_id INTEGER REFERENCES cfb_teams(team_id),
        depth_role TEXT,
        availability_status TEXT,
        availability_confidence DOUBLE PRECISION,
        attributes_json JSONB NOT NULL DEFAULT '{}'::jsonb,
        UNIQUE (snapshot_id, source_player_id)
    )
    """,

    """
    CREATE TABLE IF NOT EXISTS cfb_team_game_features (
        id BIGSERIAL PRIMARY KEY,
        game_id INTEGER NOT NULL REFERENCES cfb_matchups(id) ON DELETE CASCADE,
        team_id INTEGER NOT NULL REFERENCES cfb_teams(team_id) ON DELETE CASCADE,
        opponent_team_id INTEGER NOT NULL REFERENCES cfb_teams(team_id),
        feature_version TEXT NOT NULL,
        as_of_at TIMESTAMPTZ NOT NULL,
        available_at TIMESTAMPTZ NOT NULL,
        games_played INTEGER NOT NULL DEFAULT 0,
        effective_games DOUBLE PRECISION NOT NULL DEFAULT 0,
        current_weight DOUBLE PRECISION NOT NULL DEFAULT 0 CHECK (current_weight >= 0 AND current_weight <= 1),
        prior_weight DOUBLE PRECISION NOT NULL DEFAULT 1 CHECK (prior_weight >= 0 AND prior_weight <= 1),
        features_json JSONB NOT NULL DEFAULT '{}'::jsonb,
        source_completeness DOUBLE PRECISION CHECK (source_completeness >= 0 AND source_completeness <= 1),
        created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
        UNIQUE (game_id, team_id, feature_version, as_of_at),
    ...
    CREATE TABLE IF NOT EXISTS cfb_hypotheses (
        ...
        outcome_definition_json JSONB NOT NULL,
        population_filter_json JSONB NOT NULL,
        feature_definition_json JSONB NOT NULL DEFAULT '{}'::jsonb,
        bucket_definition_json JSONB NOT NULL DEFAULT '{}'::jsonb,
        min_sample_json JSONB NOT NULL DEFAULT '{}'::jsonb,
        split_plan_json JSONB NOT NULL DEFAULT '{}'::jsonb,
    ...
    """
    CREATE TABLE IF NOT EXISTS cfb_hypothesis_results (
        id BIGSERIAL PRIMARY KEY,
        hypothesis_id BIGINT NOT NULL REFERENCES cfb_hypotheses(id) ON DELETE CASCADE,
        ...
        UNIQUE (hypothesis_id, evaluation_type, test_start, test_end, result_payload_hash)
    )
    """,

    """
    CREATE TABLE IF NOT EXISTS cfb_game_signal_snapshots (
        id BIGSERIAL PRIMARY KEY,
        game_id INTEGER NOT NULL REFERENCES cfb_matchups(id) ON DELETE CASCADE,
        team_id INTEGER REFERENCES cfb_teams(team_id),
        hypothesis_id BIGINT NOT NULL REFERENCES cfb_hypotheses(id) ON DELETE CASCADE,
    ...
    """CREATE TABLE IF NOT EXISTS ff_source_snapshots (
        id BIGSERIAL PRIMARY KEY,
        source TEXT NOT NULL,
        dataset TEXT NOT NULL,
        season INTEGER NOT NULL,
    ...
    """CREATE TABLE IF NOT EXISTS ff_player_injury_observations (
        id BIGSERIAL PRIMARY KEY,
        player_id BIGINT NOT NULL REFERENCES ff_players(id) ON DELETE CASCADE,
        season INTEGER NOT NULL,
        source TEXT NOT NULL,
        source_snapshot_id BIGINT REFERENCES ff_source_snapshots(id) ON DELETE SET NULL,
        source_status TEXT,
        ...
        injury_started_at TIMESTAMPTZ,
        provider_updated_at TIMESTAMPTZ,
        expected_return_min DATE,
        expected_return_max DATE,
        weeks_out_min DOUBLE PRECISION,
        weeks_out_max DOUBLE PRECISION,
        availability_probability DOUBLE PRECISION,
        raw_payload JSONB NOT NULL DEFAULT '{}'::jsonb,
        response_hash TEXT NOT NULL,
        observed_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
        UNIQUE(source_snapshot_id, player_id, source),
        CHECK(normalized_status IN ('HEALTHY','QUESTIONABLE','DOUBTFUL','OUT','IR','PUP','NFI','SUSPENDED','UNKNOWN'))
    )""",
    """CREATE TABLE IF NOT EXISTS ff_player_injuries (
        id BIGSERIAL PRIMARY KEY,
        player_id BIGINT NOT NULL REFERENCES ff_players(id) ON DELETE CASCADE,
        season INTEGER NOT NULL,
        status TEXT NOT NULL,
        ...
        weeks_out_min DOUBLE PRECISION,
        weeks_out_max DOUBLE PRECISION,
        estimate_basis TEXT NOT NULL DEFAULT 'unknown',
        confidence DOUBLE PRECISION,
        primary_source TEXT NOT NULL,
        source_conflict BOOLEAN NOT NULL DEFAULT FALSE,
        active BOOLEAN NOT NULL DEFAULT TRUE,
        CHECK(status IN ('QUESTIONABLE','DOUBTFUL','OUT','IR','PUP','NFI','SUSPENDED','UNKNOWN')),
        CHECK(estimate_basis IN ('provider','return-date-derived','status-heuristic','unknown'))
    )""",
    """CREATE TABLE IF NOT EXISTS ff_injury_events (
        id BIGSERIAL PRIMARY KEY,
        injury_id BIGINT REFERENCES ff_player_injuries(id) ON DELETE CASCADE,
        player_id BIGINT NOT NULL REFERENCES ff_players(id) ON DELETE CASCADE,
        observation_id BIGINT REFERENCES ff_player_injury_observations(id) ON DELETE SET NULL,
        ...
        CHECK(event_type IN ('NEW_INJURY','STATUS_CHANGED','TIMELINE_CHANGED','PRACTICE_UPGRADE','PRACTICE_DOWNGRADE','CLEARED','RETURNED','SOURCE_CONFLICT'))
    )""",
    ...
    """CREATE TABLE IF NOT EXISTS ff_player_rankings (
        id BIGSERIAL PRIMARY KEY,
        ...
        our_rank INTEGER,
        our_projected_points DOUBLE PRECISION,
        expected_games DOUBLE PRECISION,
        confidence DOUBLE PRECISION,
        source_row JSONB,
        notes TEXT,
    ...
    """CREATE TABLE IF NOT EXISTS ff_player_season_features (
        ...
        team_target_rank INTEGER,
        team_rush_rank INTEGER,
        nfl_target_rank INTEGER,
        nfl_rush_td_rank INTEGER,
    ...
    # NFL DFS projections are kept outside the NBA/MLB dk_slates lifecycle.
    # A run records exactly what the model knew at projection time; player
    # rows are immutable children so later prop overlays can cite (rather
    # than overwrite) the historical baseline they updated.
    """CREATE TABLE IF NOT EXISTS nfl_dfs_projection_runs (
        run_id UUID PRIMARY KEY,
        model_version TEXT NOT NULL,
        scoring TEXT NOT NULL DEFAULT 'DK',
        slate_date DATE,
        season INTEGER NOT NULL,
        week INTEGER,
        as_of_at TIMESTAMPTZ NOT NULL,
        seed BIGINT NOT NULL,
        history_cutoff_season INTEGER NOT NULL,
        ...
        created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
        UNIQUE(model_version, artifact_digest),
        CHECK(scoring = 'DK'),
    ...
    """CREATE TABLE IF NOT EXISTS nfl_dfs_player_projections (
        ...
        model_proj_fpts DOUBLE PRECISION,
        baseline_fpts DOUBLE PRECISION,
        floor_fpts DOUBLE PRECISION,
        median_fpts DOUBLE PRECISION,
        ceiling_fpts DOUBLE PRECISION,
        boom_rate DOUBLE PRECISION,
        confidence DOUBLE PRECISION NOT NULL,
        stat_means JSONB NOT NULL DEFAULT '{}'::jsonb,
        ...
        CHECK(boom_rate IS NULL OR (boom_rate >= 0 AND boom_rate <= 1))
    )""",
    """CREATE TABLE IF NOT EXISTS nfl_dfs_slate_uploads (
    ...
    """CREATE TABLE IF NOT EXISTS nfl_dfs_slate_players (
        ...
        dk_player_id BIGINT NOT NULL,
        captain_dk_player_id BIGINT,
        ff_player_id BIGINT REFERENCES ff_players(id),
        name TEXT NOT NULL,
        normalized_name TEXT NOT NULL,
        position TEXT NOT NULL,
        ...
        fantasypros_proj DOUBLE PRECISION,
        linestar_proj DOUBLE PRECISION,
        linestar_own_pct DOUBLE PRECISION,
        custom_proj DOUBLE PRECISION,
    ...
    """CREATE TABLE IF NOT EXISTS nfl_dfs_optimizer_runs (
        ...
        requested_lineups INTEGER NOT NULL,
        generated_lineups INTEGER NOT NULL,
        status TEXT NOT NULL,
        failure_reason TEXT,
        created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    ...
    """CREATE TABLE IF NOT EXISTS nfl_dfs_lineups (
        id BIGSERIAL PRIMARY KEY,
        run_id UUID NOT NULL REFERENCES nfl_dfs_optimizer_runs(run_id) ON DELETE CASCADE,
        lineup_number INTEGER NOT NULL,
        slots JSONB NOT NULL,
        player_ids JSONB NOT NULL,
        total_salary INTEGER NOT NULL,
        projected_fpts DOUBLE PRECISION NOT NULL,
        floor_fpts DOUBLE PRECISION,
        ceiling_fpts DOUBLE PRECISION,
        projected_ownership DOUBLE PRECISION,
        stack_summary JSONB NOT NULL DEFAULT '{}'::jsonb,
        actual_fpts DOUBLE PRECISION,
        created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
        UNIQUE(run_id, lineup_number)
    )""",
    ...
    """CREATE TABLE IF NOT EXISTS ff_draft_teams (
        id BIGSERIAL PRIMARY KEY,
        draft_id UUID NOT NULL REFERENCES ff_draft_sessions(id) ON DELETE CASCADE,
    ...
    """CREATE TABLE IF NOT EXISTS ff_draft_slots (
        id BIGSERIAL PRIMARY KEY,
        draft_id UUID NOT NULL REFERENCES ff_draft_sessions(id) ON DELETE CASCADE,
    ...
    """CREATE TABLE IF NOT EXISTS ff_draft_events (
        id BIGSERIAL PRIMARY KEY,
        draft_id UUID NOT NULL REFERENCES ff_draft_sessions(id) ON DELETE CASCADE,
        event_type TEXT NOT NULL,
        ...
        created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
        CHECK(event_type IN ('pick_made', 'pick_reversed', 'draft_started', 'draft_paused', 'draft_resumed', 'draft_completed', 'settings_changed')),
        CHECK(source IN ('manual', 'sleeper', 'system'))
    )""",
    """CREATE TABLE IF NOT EXISTS ff_draft_player_preferences (
        draft_id UUID NOT NULL REFERENCES ff_draft_sessions(id) ON DELETE CASCADE,
        player_id BIGINT NOT NULL REFERENCES ff_players(id),
    ...
    # Roster-aware V2 historical context is append-only by deterministic run.
    # A corrected upstream source produces a different run id instead of
    # overwriting the rows used by an earlier validation artifact.
    """CREATE TABLE IF NOT EXISTS ff_v2_context_runs (
        run_id UUID PRIMARY KEY,
        ...
        artifact_digest TEXT NOT NULL,
        created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
        UNIQUE(transform_version, artifact_digest)
    )""",
    """CREATE TABLE IF NOT EXISTS ff_v2_team_week_context (
        id BIGSERIAL PRIMARY KEY,
        run_id UUID NOT NULL REFERENCES ff_v2_context_runs(run_id) ON DELETE CASCADE,
        season INTEGER NOT NULL,
        week INTEGER NOT NULL,
        team TEXT NOT NULL,
        ...
        roof TEXT,
        surface TEXT,
        quarterback_gsis_id TEXT,
        quarterback_name TEXT,
        head_coach TEXT,
        play_caller_id TEXT,
        source_snapshot_id BIGINT NOT NULL REFERENCES ff_source_snapshots(id),
        row_digest TEXT NOT NULL,
        observed_at TIMESTAMPTZ NOT NULL,
        UNIQUE(run_id, season, week, team)
    )""",
    """CREATE TABLE IF NOT EXISTS ff_v2_roster_weeks (
        id BIGSERIAL PRIMARY KEY,
        run_id UUID NOT NULL REFERENCES ff_v2_context_runs(run_id) ON DELETE CASCADE,
        season INTEGER NOT NULL,
        week INTEGER NOT NULL,
        ...
        row_digest TEXT NOT NULL,
        observed_at TIMESTAMPTZ NOT NULL,
        UNIQUE(run_id, season, week, player_gsis_id)
    )""",
    """CREATE TABLE IF NOT EXISTS ff_v2_transactions (
        ...
        row_digest TEXT NOT NULL,
        observed_at TIMESTAMPTZ NOT NULL,
        UNIQUE(run_id, player_gsis_id, effective_at, to_team)
    )""",
    """CREATE TABLE IF NOT EXISTS ff_v2_team_week_facts (
        id BIGSERIAL PRIMARY KEY,
        run_id UUID NOT NULL REFERENCES ff_v2_context_runs(run_id) ON DELETE CASCADE,
        season INTEGER NOT NULL,
        week INTEGER NOT NULL,
        ...
        allocatable_targets INTEGER NOT NULL,
        rush_attempts INTEGER NOT NULL,
        rb_carries INTEGER NOT NULL,
        rb_targets INTEGER NOT NULL,
        pass_touchdowns INTEGER NOT NULL,
        rush_touchdowns INTEGER NOT NULL,
        red_zone_trips INTEGER NOT NULL,
        goal_line_carries INTEGER NOT NULL,
        end_zone_targets INTEGER NOT NULL,
        neutral_pass_rate DOUBLE PRECISION,
        seconds_per_play DOUBLE PRECISION,
        score_state_features JSONB NOT NULL DEFAULT '{}'::jsonb,
        quarterback_gsis_id TEXT,
        quarterback_name TEXT,
        play_caller_id TEXT,
        source_snapshot_ids JSONB NOT NULL,
        ...
        observed_at TIMESTAMPTZ NOT NULL,
        UNIQUE(run_id, game_id, team)
    )""",
    # V2 team-opportunity forecasts are immutable, versioned artifacts. The
    # long-form distribution table keeps every forecast pool under one shared
    # contract and avoids silently changing schema as later pools are added.
    """CREATE TABLE IF NOT EXISTS ff_v2_team_opportunity_forecast_runs (
        run_id UUID PRIMARY KEY,
        contract_version TEXT NOT NULL,
        context_run_id UUID NOT NULL REFERENCES ff_v2_context_runs(run_id),
        model_version TEXT NOT NULL,
        ...
        artifact_digest TEXT NOT NULL,
        created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
        UNIQUE(contract_version, artifact_digest)
    )""",
    """CREATE TABLE IF NOT EXISTS ff_v2_team_opportunity_forecasts (
        id BIGSERIAL PRIMARY KEY,
        forecast_run_id UUID NOT NULL REFERENCES ff_v2_team_opportunity_forecast_runs(run_id) ON DELETE CASCADE,
        context_fact_id BIGINT NOT NULL REFERENCES ff_v2_team_week_facts(id),
        context_fact_digest TEXT NOT NULL,
        season INTEGER NOT NULL,
        week INTEGER NOT NULL,
        game_id TEXT NOT NULL,
        game_date DATE NOT NULL,
        ...
        as_of_at TIMESTAMPTZ NOT NULL,
        created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
        UNIQUE(forecast_run_id, game_id, team),
        CHECK(fallback_tier IN ('A', 'B', 'C')),
        CHECK(confidence_multiplier > 0 AND confidence_multiplier <= 1)
    )""",
    """CREATE TABLE IF NOT EXISTS ff_v2_team_opportunity_distributions (
        id BIGSERIAL PRIMARY KEY,
        forecast_id BIGINT NOT NULL REFERENCES ff_v2_team_opportunity_forecasts(id) ON DELETE CASCADE,
        ...
        UNIQUE(forecast_id, opportunity_type),
        CHECK(opportunity_type IN ('plays', 'pass_attempts', 'allocatable_targets', 'rush_attempts', 'rb_carries', 'rb_targets', 'pass_touchdowns', 'rush_touchdowns')),
        CHECK(expected_value >= 0 AND dispersion >= 0),
        CHECK(p10 >= 0 AND p10 <= p50 AND p50 <= p90)
    )""",
    """CREATE TABLE IF NOT EXISTS ff_v2_backtest_runs (
        run_id UUID PRIMARY KEY,
        ...
        calibration_version TEXT NOT NULL,
        seed BIGINT NOT NULL,
        evaluation_seasons JSONB NOT NULL,
        preseason_cutoffs JSONB NOT NULL,
        source_snapshot_ids JSONB NOT NULL,
        ...
        created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
        UNIQUE(harness_version, output_digest)
    )""",
    """CREATE TABLE IF NOT EXISTS ff_v2_backtest_splits (
        id BIGSERIAL PRIMARY KEY,
        run_id UUID NOT NULL REFERENCES ff_v2_backtest_runs(run_id) ON DELETE CASCADE,
        evaluation_season INTEGER NOT NULL,
        preseason_cutoff TIMESTAMPTZ NOT NULL,
        training_seasons JSONB NOT NULL,
        training_row_counts JSONB NOT NULL,
        evaluation_row_counts JSONB NOT NULL,
        training_digest TEXT NOT NULL,
        evaluation_digest TEXT NOT NULL,
        split_digest TEXT NOT NULL,
    ...
    CREATE TABLE IF NOT EXISTS nfl_pbp_archetypes (
        ...
        drive_inside_twenty DOUBLE PRECISION,
        wiped_event TEXT,
        wiped_yards DOUBLE PRECISION,
        wiped_touchdown BOOLEAN,
        wiped_turnover BOOLEAN,
        wiped_sack BOOLEAN,
        wiped_defender TEXT,
        drive_injuries INTEGER,
        outcome TEXT,
        converted BOOLEAN,
        scramble BOOLEAN,
        two_point_result TEXT,
        drive_no_first_down BOOLEAN,
        drive_score_against_mechanism TEXT,
        defteam TEXT,
        posteam_type TEXT,
        div_game BOOLEAN,
        ...
        pass_rushers DOUBLE PRECISION,
        blitz BOOLEAN,
        heavy_blitz BOOLEAN,
        n_ol DOUBLE PRECISION,
        n_wr DOUBLE PRECISION,
        pressure BOOLEAN,
        coverage_type TEXT,
        man_zone TEXT,
        ...
        drive_result TEXT,
        drive_turnover_type TEXT,
        drive_had_sack BOOLEAN,
        drive_had_penalty BOOLEAN,
        drive_failed_short BOOLEAN,
        drive_plays INTEGER,
        drive_net_yards DOUBLE PRECISION,
        drive_epa DOUBLE PRECISION,
        play_labeller_version TEXT NOT NULL,
        drive_labeller_version TEXT,
        participation_labeller_version TEXT NOT NULL,
...
MIGRATIONS = [
    ...
    "CREATE UNIQUE INDEX IF NOT EXISTS idx_cfb_team_features_snapshot_identity ON cfb_team_game_features(game_id, team_id, feature_version, as_of_at)",
    ...
    # play-v3: goal_line replaces the GOAL_LINE_PUNCH label, penalty gains
    # attributes, and the participation release lands (formation, personnel,
    # box, rushers, pressure, coverage).
    "ALTER TABLE nfl_pbp_archetypes ADD COLUMN IF NOT EXISTS goal_line BOOLEAN",
    "ALTER TABLE nfl_pbp_archetypes ADD COLUMN IF NOT EXISTS penalty_type TEXT",
    "ALTER TABLE nfl_pbp_archetypes ADD COLUMN IF NOT EXISTS penalty_team TEXT",
    "ALTER TABLE nfl_pbp_archetypes ADD COLUMN IF NOT EXISTS penalty_first_down BOOLEAN",
    "ALTER TABLE nfl_pbp_archetypes ADD COLUMN IF NOT EXISTS formation TEXT",
    "ALTER TABLE nfl_pbp_archetypes ADD COLUMN IF NOT EXISTS personnel_grouping TEXT",
    "ALTER TABLE nfl_pbp_archetypes ADD COLUMN IF NOT EXISTS defenders_in_box DOUBLE PRECISION",
    "ALTER TABLE nfl_pbp_archetypes ADD COLUMN IF NOT EXISTS participation_labeller_version TEXT",
    "ALTER TABLE nfl_pbp_archetypes ADD COLUMN IF NOT EXISTS converted BOOLEAN",
    ...
    "ALTER TABLE nfl_pbp_archetypes ADD COLUMN IF NOT EXISTS penalty_yards DOUBLE PRECISION",
    "ALTER TABLE nfl_pbp_archetypes ADD COLUMN IF NOT EXISTS air_yards DOUBLE PRECISION",
    "ALTER TABLE nfl_pbp_archetypes ADD COLUMN IF NOT EXISTS yards_after_catch DOUBLE PRECISION",
    "ALTER TABLE nfl_pbp_archetypes ADD COLUMN IF NOT EXISTS xyac_mean_yardage DOUBLE PRECISION",
    "ALTER TABLE nfl_pbp_archetypes ADD COLUMN IF NOT EXISTS pass_length TEXT",
    "ALTER TABLE nfl_pbp_archetypes ADD COLUMN IF NOT EXISTS pass_location TEXT",
    "ALTER TABLE nfl_pbp_archetypes ADD COLUMN IF NOT EXISTS run_location TEXT",
    "ALTER TABLE nfl_pbp_archetypes ADD COLUMN IF NOT EXISTS run_gap TEXT",
    "ALTER TABLE nfl_pbp_archetypes ADD COLUMN IF NOT EXISTS cp DOUBLE PRECISION",
    "ALTER TABLE nfl_pbp_archetypes ADD COLUMN IF NOT EXISTS cpoe DOUBLE PRECISION",
    "ALTER TABLE nfl_pbp_archetypes ADD COLUMN IF NOT EXISTS xpass DOUBLE PRECISION",
    "ALTER TABLE nfl_pbp_archetypes ADD COLUMN IF NOT EXISTS pass_oe DOUBLE PRECISION",
    "ALTER TABLE nfl_pbp_archetypes ADD COLUMN IF NOT EXISTS series INTEGER",
    "ALTER TABLE nfl_pbp_archetypes ADD COLUMN IF NOT EXISTS series_success BOOLEAN",
    "ALTER TABLE nfl_pbp_archetypes ADD COLUMN IF NOT EXISTS series_result TEXT",
    "ALTER TABLE nfl_pbp_archetypes ADD COLUMN IF NOT EXISTS goal_to_go BOOLEAN",
    "ALTER TABLE nfl_pbp_archetypes ADD COLUMN IF NOT EXISTS out_of_bounds BOOLEAN",
    "ALTER TABLE nfl_pbp_archetypes ADD COLUMN IF NOT EXISTS timeout_team TEXT",
    "ALTER TABLE nfl_pbp_archetypes ADD COLUMN IF NOT EXISTS home_coach TEXT",
    "ALTER TABLE nfl_pbp_archetypes ADD COLUMN IF NOT EXISTS away_coach TEXT",
    "ALTER TABLE nfl_pbp_archetypes ADD COLUMN IF NOT EXISTS drive_time_of_possession TEXT",
    "ALTER TABLE nfl_pbp_archetypes ADD COLUMN IF NOT EXISTS drive_yards_penalized DOUBLE PRECISION",
    "ALTER TABLE nfl_pbp_archetypes ADD COLUMN IF NOT EXISTS drive_inside_twenty DOUBLE PRECISION",
    "ALTER TABLE nfl_pbp_archetypes ADD COLUMN IF NOT EXISTS wiped_event TEXT",
    "ALTER TABLE nfl_pbp_archetypes ADD COLUMN IF NOT EXISTS wiped_yards DOUBLE PRECISION",
    "ALTER TABLE nfl_pbp_archetypes ADD COLUMN IF NOT EXISTS wiped_touchdown BOOLEAN",
    "ALTER TABLE nfl_pbp_archetypes ADD COLUMN IF NOT EXISTS wiped_turnover BOOLEAN",
    "ALTER TABLE nfl_pbp_archetypes ADD COLUMN IF NOT EXISTS wiped_sack BOOLEAN",
    "ALTER TABLE nfl_pbp_archetypes ADD COLUMN IF NOT EXISTS wiped_defender TEXT",
    "ALTER TABLE nfl_pbp_archetypes ADD COLUMN IF NOT EXISTS drive_injuries INTEGER",
    ...
    "ALTER TABLE nfl_pbp_archetypes ADD COLUMN IF NOT EXISTS posteam_type TEXT",
    "ALTER TABLE nfl_pbp_archetypes ADD COLUMN IF NOT EXISTS div_game BOOLEAN",
    "ALTER TABLE nfl_pbp_archetypes ADD COLUMN IF NOT EXISTS score_differential INTEGER",
    "ALTER TABLE nfl_pbp_archetypes ADD COLUMN IF NOT EXISTS game_seconds_remaining INTEGER",
    "ALTER TABLE nfl_pbp_archetypes ADD COLUMN IF NOT EXISTS half_seconds_remaining INTEGER",
    "ALTER TABLE nfl_pbp_archetypes ADD COLUMN IF NOT EXISTS roof TEXT",
    "ALTER TABLE nfl_pbp_archetypes ADD COLUMN IF NOT EXISTS surface TEXT",
    "ALTER TABLE nfl_pbp_archetypes ADD COLUMN IF NOT EXISTS temp DOUBLE PRECISION",
    "ALTER TABLE nfl_pbp_archetypes ADD COLUMN IF NOT EXISTS wind DOUBLE PRECISION",
    "ALTER TABLE nfl_pbp_archetypes ADD COLUMN IF NOT EXISTS spread_line DOUBLE PRECISION",
    "ALTER TABLE nfl_pbp_archetypes ADD COLUMN IF NOT EXISTS total_line DOUBLE PRECISION",
    "ALTER TABLE nfl_pbp_archetypes ADD COLUMN IF NOT EXISTS qb_dropback BOOLEAN",
    "ALTER TABLE nfl_pbp_archetypes ADD COLUMN IF NOT EXISTS first_down BOOLEAN",
    "ALTER TABLE nfl_pbp_archetypes ADD COLUMN IF NOT EXISTS tackled_for_loss BOOLEAN",
    "ALTER TABLE nfl_pbp_archetypes ADD COLUMN IF NOT EXISTS st_outcome TEXT",
    "ALTER TABLE nfl_pbp_archetypes ADD COLUMN IF NOT EXISTS kick_distance DOUBLE PRECISION",
    "ALTER TABLE nfl_pbp_archetypes ADD COLUMN IF NOT EXISTS return_yards DOUBLE PRECISION",
    "ALTER TABLE nfl_pbp_archetypes ADD COLUMN IF NOT EXISTS pass_rushers DOUBLE PRECISION",
    "ALTER TABLE nfl_pbp_archetypes ADD COLUMN IF NOT EXISTS blitz BOOLEAN",
    "ALTER TABLE nfl_pbp_archetypes ADD COLUMN IF NOT EXISTS heavy_blitz BOOLEAN",
    "ALTER TABLE nfl_pbp_archetypes ADD COLUMN IF NOT EXISTS n_ol DOUBLE PRECISION",
    "ALTER TABLE nfl_pbp_archetypes ADD COLUMN IF NOT EXISTS n_wr DOUBLE PRECISION",
    "ALTER TABLE nfl_pbp_archetypes ADD COLUMN IF NOT EXISTS pressure BOOLEAN",
    "ALTER TABLE nfl_pbp_archetypes ADD COLUMN IF NOT EXISTS coverage_type TEXT",
    "ALTER TABLE nfl_pbp_archetypes ADD COLUMN IF NOT EXISTS man_zone TEXT",
...
INDEXES = [
    ...
    "CREATE INDEX IF NOT EXISTS idx_cfb_history_game_market ON cfb_historical_game_lines(game_id, market_type, line_designation)",
    "CREATE INDEX IF NOT EXISTS idx_cfb_history_canonical_spread ON cfb_historical_game_lines(home_value, game_id) WHERE market_type='spread' AND is_canonical_reference=TRUE",
    "CREATE INDEX IF NOT EXISTS idx_cfb_staff_regime_team ON cfb_staff_regimes(team_id, start_season, end_season)",
    "CREATE INDEX IF NOT EXISTS idx_cfb_roster_snapshot_asof ON cfb_roster_snapshots(team_id, season, available_at DESC)",
    "CREATE INDEX IF NOT EXISTS idx_cfb_roster_players_snapshot ON cfb_roster_players(snapshot_id, position_group)",
    "CREATE INDEX IF NOT EXISTS idx_cfb_team_features_asof ON cfb_team_game_features(team_id, as_of_at DESC, feature_version)",
